...
class GasCar(Vehicle):
    def __init__(self, name: str, base_fee: float, liters_per_100km: float, price_per_liter: float):
...
class ElectricCar(Vehicle):
    def __init__(self, name: str, base_fee: float, kwh_per_100km: float, price_per_kwh: float):
        super().__init__(name, base_fee)
...
class Taxi(Vehicle):
    def __init__(self, name: str, base_fee, price_per_km: float, is_night: bool):
        super().__init__(name, base_fee)
...
if __name__ == "__main__":
    car1 = GasCar("GasCar", 10, 8, 1.5)
    car2 = ElectricCar("ElectricCar", 10, 15, 0.5)
    car3 = Taxi("Taxi", 10, 1.5, True)
    car4 = Taxi("Taxi", 10, 1.5, False)
